...
def generate_mono_keyboard(mono_id):
    ...
    kb_rows.append([InlineKeyboardButton(text="🗑 Ігнорувати (Не записувати)", callback_data=f"mdel:{mono_id}")])
...
async def mono_webhook_get(request):
    """Перевірочний запит від Монобанку."""
...
async def mono_webhook_post(request):
    ...
    if data.get("type") == "StatementItem":
        ...
        amount = statement.get("amount", 0) / 100.0
...
async def full_save(message: types.Message, state: FSMContext):
    ...
    await run_sync(
        ...
        data['category'], data['amount'], data['t_type'], message.text, "Bot", who
    )
...
async def handle_quick_action(message: types.Message):
    text = message.text
    if "Кава" in text: amount, cat, desc = 60, "☕ Щоденні дрібниці", "Кава (Швидкий запис)"
    elif "Енергетик" in text: amount, cat, desc = 45, "☕ Щоденні дрібниці", "Енергетик (Швидкий запис)"
    else: amount, cat, desc = 8, "🚆 Громад. транспорт", "Метро (Швидкий запис)"
    ...
    if res:
        await message.answer(f"✅ Швидко записано: <b>{amount} грн</b> ({cat})\n<i>Якщо сума інша — натисни кнопку нижче.</i>", parse_mode="HTML", reply_markup=quick_edit_kb())
    else:
...
async def qa_cancel(call: types.CallbackQuery):
    ...
    if res: await call.message.edit_text(f"❌ Запис на {res['amount']} скасовано.")
    else: await call.answer("Помилка видалення", show_alert=True)
...
async def qa_edit_finish(message: types.Message, state: FSMContext):
    try: new_amt = float(message.text.replace(',', '.'))
    except ValueError: return await message.answer("❌ Це не число. Спробуй ще раз.")
    await bot.send_chat_action(message.chat.id, ChatAction.TYPING)
    res = await run_sync(gs.update_last_transaction_amount, new_amt)
    if res: await message.answer(f"✅ Суму успішно змінено на <b>{new_amt} грн</b>!", parse_mode="HTML")
    else: await message.answer("❌ Помилка оновлення таблиці.")
    await state.clear()

@dp.message(F.text == "💸 Переказ", StateFilter("*"))
async def transfer_start(message: types.Message, state: FSMContext):
    await state.clear()
    kb = [[KeyboardButton(text="🧔‍♂️ Вадим ➡️ 👩‍🦰 Аня")], [KeyboardButton(text="👩‍🦰 Аня ➡️ 🧔‍♂️ Вадим")], [KeyboardButton(text="🔙 Назад")]]
    await state.set_state(TransferForm.direction)
    await message.answer("Хто і кому передає кошти?", reply_markup=ReplyKeyboardMarkup(keyboard=kb, resize_keyboard=True))

@dp.message(TransferForm.direction)
async def transfer_dir(message: types.Message, state: FSMContext):
    if "Вадим ➡️" in message.text: await state.update_data(from_who="Вадим", to_who="Аня")
    elif "Аня ➡️" in message.text: await state.update_data(from_who="Аня", to_who="Вадим")
    else: return await message.answer("Обери кнопку.")
...
async def transfer_save(message: types.Message, state: FSMContext):
    data = await state.get_data()
    await bot.send_chat_action(message.chat.id, ChatAction.TYPING)
    res = await run_sync(gs.add_transfer, data['amount'], message.text, data['from_who'], data['to_who'])
    if res:
        await message.answer(f"✅ <b>Успішно!</b>\n📤 {data['from_who']}: -{data['amount']} грн\n📥 {data['to_who']}: +{data['amount']} грн\n💬 <i>{message.text}</i>", reply_markup=main_kb(), parse_mode="HTML")
    else:
...
async def render_history(message: types.Message, page=1, is_edit=False):
    await bot.send_chat_action(message.chat.id, ChatAction.TYPING)
    last_txs = await run_sync(gs.get_last_transactions, page, 5)
    ...
    for tx in last_txs:
        ...
        buttons.append(InlineKeyboardButton(text=f"🗑 Вид. ({tx['amount']})", callback_data=f"ask_del:{tx['id']}:{page}"))
        text += "➖➖➖➖➖➖\n"
    nav_row = []
    if page > 1: nav_row.append(InlineKeyboardButton(text="⬅️ Назад", callback_data=f"hist_page:{page - 1}"))
    if len(last_txs) == 5: nav_row.append(InlineKeyboardButton(text="Вперед ➡️", callback_data=f"hist_page:{page + 1}"))
...
async def history_nav(callback: CallbackQuery):
    await render_history(callback.message, page=int(callback.data.split(":")[1]), is_edit=True)
...
async def ask_delete(callback: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅ Так", callback_data=f"do_del:{tx_id}:{page}"), InlineKeyboardButton(text="❌ Ні", callback_data=f"hist_page:{page}")]])
    await callback.message.edit_text("⚠️ <b>Ви справді хочете видалити цей запис?</b>\nЦе неможливо скасувати.", reply_markup=kb, parse_mode="HTML")
...
async def report_year(message: types.Message, state: FSMContext):
    if message.text == "🔙 Назад": return await show_other_menu(message)
    try:
    ...
    except: await message.answer("Цифрами!")

@dp.message(ReportForm.month)
async def report_month(message: types.Message, state: FSMContext):
    if message.text == "🔙 Назад": return await report_start(message, state)
    month_map = {"Січень": 1, "Лютий": 2, "Березень": 3, "Квітень": 4, "Травень": 5, "Червень": 6, "Липень": 7, "Серпень": 8, "Вересень": 9, "Жовтень": 10, "Листопад": 11, "Грудень": 12}
    if message.text not in month_map: return await message.answer("Обери кнопку")
    
    data_state = await state.get_data()
    year, month = data_state['year'], month_map[message.text]
    ...
    try:
        ...
        await message.answer_document(BufferedInputFile(pdf.read(), filename=f"Report_{message.text}_{year}.pdf"), caption=f"📊 Звіт за <b>{message.text} {year}</b> готовий!", parse_mode="HTML")
...
async def curr_start(message: types.Message, state: FSMContext):
    await state.update_data(op=message.text)
...
async def curr_save(message: types.Message, state: FSMContext):
    data = await state.get_data()
    usd, rate, op = data['amount_usd'], data['rate'], data['op']
    who = USERS.get(message.from_user.id, "Unknown")
    ...
    await run_sync(gs.add_currency_transaction, date, op, usd_sign, rate, uah_val, message.text)
...
async def send_chart(message: types.Message):
    ...
    if img_buf: await message.answer_photo(BufferedInputFile(img_buf.read(), filename="chart.png"), caption=f"Витрати за {stats['month_name']}")
...
async def edit_limit_save(message: types.Message, state: FSMContext):
    try:
        ...
        else: await message.answer("❌ Помилка", reply_markup=other_kb())
...
async def check_daily_reminders():
    ...
    for item in reminders:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅ Сплатити", callback_data=f"pay_rem:{row_idx}")]])
        for uid in USERS:
            try: await bot.send_message(uid, text, reply_markup=kb, parse_mode="HTML")
            except: pass
...
async def pay_reminder(callback: CallbackQuery):
    ...
    await asyncio.gather(
        run_sync(gs.add_transaction, date_now, item['category'], item['amount'], "Витрати", item['name'], "Auto-Reminder", who),
